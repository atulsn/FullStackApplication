...
def reg(request):
    if request.method == "POST":
        ...
        return JsonResponse({"message":"Registered Successfull"},status = 201)
...
def login(request):
    if request.method == "POST":
        ...
        if user:
            ...
            return JsonResponse({"message":"Invalid Email or Passsword"})
...
def get_data(request):
    ...
    return JsonResponse({"Details":"GET method only"})
